...
*/ 
void ZnakiPuste(int Ile = 0) { 
  //ZnakiPuste - Funkcja wyświetla określoną ilość znaków pustych. 
    for(int I = 0; I < Ile; I++) { cout << " "; } 
} 
int iNadluzszyCiagZnakow(string Str = "") { 
  //iNadluzszyCiagZnakow. 
...
void LiniaPozioma(int IleZnakow = 0) { 
  //LiniaPozioma - Rysuje linię poziomą. 
    if(IleZnakow > 0) { 
      cout << "+"; 
      for(int I = 0; I < IleZnakow; I++) { cout << "-"; } 
      cout << "+"; 
    } 
} 
...
/* 
  Legenda: 
    ...
    2) Jeżeli warunek "if(Str[I] == ';')" jest niespełniony, to dodaj do zmiennej 
       tekstowej "T1" kolejne odczytane znaki ze zmiennej tekstowej "Str". 
*/ 
} 
  ...
  //Znajdź najdłuższy ciąg znaków. 
    ...
    if(PlikDane != NULL) { 
      ...
      while(getline(PlikDane, Linia)) { 
        Ile = iNadluzszyCiagZnakow(Linia+';'); 
