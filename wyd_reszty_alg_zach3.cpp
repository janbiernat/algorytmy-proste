#include <iostream>
/*--== Wydawanie reszty algorytmem zach³annym 3, Zabezpieczenie ==--
...
4. Wróæ do kroku 2 i 3.
   ...
   równa zero.
=
Zabezpieczenie algorytmu polega
na za³o¿eniu, ¿e w kasie zawsze
s¹ nomina³y o wartoœci 1z³.
*/
using namespace std;
int main() {
  cout << "--== Wydawanie reszty algorytmem zachlannym 3, Zabezpieczenie ==-- \n \n";
  ...
  if(ResztaDoWydania > 0) {
    ...
    int Nominaly[IloscNominalow+1]; //1.

    for(int I = 0; I < IloscNominalow; I++) {
      cout << "Nominal " << (I+1) << ": ";
      cin >> Nominaly[I];
      if(Nominaly[I] < 2) { Nominaly[I] = 2; }
    }

    //Zabezpieczenie algorytmu.
    Nominaly[IloscNominalow] = 1;
    cout << "\nNominal: " << Nominaly[IloscNominalow] << " <- ZABEZPIECZENIE";
